  ...
  return (
    <div className={`p-8 sm:p-10 bg-white border border-[#17191A]/10 rounded-sm shadow-xs ${className}`}>
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-[#17191A]/10">
        <div>
          <span className="font-mono text-[10px] uppercase font-bold tracking-widest text-[#3D9D99] block mb-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative">
        {stages.map((stage, idx) => (
          <FadeIn key={stage.step} direction="up" delay={idx * 0.1} className="h-full">
            <div className="p-6 bg-[#F7F7F5] border border-[#17191A]/10 rounded-sm hover:border-[#3D9D99] transition-all h-full flex flex-col justify-between relative group">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="font-mono text-xs font-bold text-[#3D9D99]">
                    STAGE // {stage.step}
                  </span>
                ...
                <h4 className="font-display text-base font-semibold text-[#17191A] mb-2 group-hover:text-[#3D9D99] transition-colors">
                  {stage.name}
                </h4>
