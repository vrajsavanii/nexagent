    ...
    <section id="features" className="w-full py-24 bg-[#FBF5F3]">
      <div className="max-w-6xl mx-auto px-6 flex flex-col items-center">
          ...
          {featureDetails.map((item, index) => (
            <motion.div
              ...
              className={`p-6 rounded-3xl cursor-pointer transition-all ${
                activeTab === index
                  ? 'bg-white border-2 border-[#9E7B78]/40 shadow-[0_12px_28px_-4px_rgba(158,123,120,0.12)]'
            ...
            >
              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center mb-4 transition-colors ${
                activeTab === index ? 'bg-[#9E7B78] text-white' : 'bg-[rgba(158,123,120,0.10)] text-[#9E7B78]'
              }`}>
                {item.icon}
              </div>
        ...
        <div className="relative w-full max-w-2xl mt-4 flex flex-col items-center">
              ...
              {[0, 1, 2, 3].map((i) => {
                ...
                return (
                  <path
                    ...
                    stroke={active ? '#9E7B78' : 'rgba(205,211,219,0.5)'}
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-[#2A2B2E] flex items-center justify-center shadow-2xs">
              <svg viewBox="0 0 24 24" className="w-5 h-5 text-[#9E7B78]" fill="none" stroke="currentColor" strokeWidth={2}>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <p className="text-xs font-bold text-[#9E7B78] uppercase tracking-widest mb-3">
            ...
            <ul className="space-y-2">
              {featureDetails[activeTab].details.map((d, i) => (
                <li key={i} className="flex items-center gap-2.5 text-sm text-[#5E6572]">
                  <span className="w-4 h-4 rounded-full bg-[rgba(158,123,120,0.10)] text-[#9E7B78] flex items-center justify-center text-[10px] font-black flex-shrink-0">✓</span>
                  {d}
                </li>
              ))}
            </ul>
