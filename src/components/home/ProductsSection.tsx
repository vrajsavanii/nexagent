  ...
  return (
    <section className="relative w-full py-20 lg:py-32 bg-surface-ground border-b border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionHeader
          badgeText="11 / PRODUCTS & PLATFORMS"
          badgeVariant="teal"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {PRODUCTS_DATA.map((product) => {
            ...
            return (
              <div
                key={product.id}
                className="p-7 rounded-2xl bg-white border border-slate-200/90 shadow-2xs hover:border-brand-400 hover:shadow-premium transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  ...
                  <div className="pt-3 border-t border-slate-100 mb-4">
                    ...
                    <ul className="flex flex-col gap-1.5">
                      {product.capabilities.map((cap, idx) => (
                        <li key={idx} className="font-sans text-xs text-slate-700 flex items-center gap-2">
